...
def test_calibration_results(
    finetune_dir: str, calibrated_dir: str, n_bins: int = 10, plot: bool = False
):
    """
    ...
    prob_true_orig, prob_pred_orig = calibration_curve(
        merged_df["targets_orig"], merged_df["probas_orig"], n_bins=n_bins
    )

    prob_true_cal, prob_pred_cal = calibration_curve(
        merged_df["targets_cal"], merged_df["probas_cal"], n_bins=n_bins
    )
...
if __name__ == "__main__":
    ...
    results = test_calibration_results(
        args.finetune_dir, args.calibrated_dir, plot=args.plot
    )
