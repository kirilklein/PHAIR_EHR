...
from corebehrt.constants.causal.data import EXPOSURE, OUTCOME, CF_OUTCOME, PROBAS, TARGETS
...
def check_predictions_match_pids(
    ...
    predictions_path = join(
        fold_dir, 
        CHECKPOINTS_DIR, 
        f"{PROBAS}_{mode}_{pred_type}_{epoch}.npz"
    )
    ...
    targets_path = join(
        fold_dir, 
        CHECKPOINTS_DIR, 
        f"{TARGETS}_{mode}_{pred_type}_{epoch}.npz"
    )
    ...
    if len(predictions) != len(pids):
        print(f"Error: Predictions length ({len(predictions)}) doesn't match PIDs length ({len(pids)})")
        return False
    
    if targets is not None and len(targets) != len(pids):
        print(f"Error: Targets length ({len(targets)}) doesn't match PIDs length ({len(pids)})")
        return False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validate consistency between fold PIDs and predictions")
