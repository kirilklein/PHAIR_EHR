...
def test_roc_performance(
    ...
    try:
        ...
        if min_roc is not None:
            ...
            if below_min:
                print(f"❌FAIL: {metric_name} value {roc_value:.4f} below minimum {min_roc}")
                passed = False
        
        if max_roc is not None:
            above_max = roc_value > max_roc
            if above_max:
                print(f"❌FAIL: {metric_name} value {roc_value:.4f} above maximum {max_roc}")
                passed = False
...
def main():
    ...
    parser.add_argument(
        "path", 
        help="Path to directory containing performance metrics"
    )
    parser.add_argument(
        "--min", 
        type=float, 
    ...
    parser.add_argument(
        ...
        type=float, 
        default=1,
        help="Maximum acceptable value (default: 1)"
    )
    parser.add_argument(
        "--metric-name", 
        type=str, 
