...
@REM python -m corebehrt.main_causal.select_cohort_full --config_path corebehrt\configs\causal\select_cohort_full\extract_uncensored.yaml
...
python tests\pipeline\test_uncensored_prepared.py --processed_data_dir .\outputs\causal\finetune\prepared_data_uncensored --tolerance 0.06
if errorlevel 1 goto :error
...
python -m tests.pipeline.test_performance_multitarget .\outputs\causal\finetune\models\uncensored --target-bounds "exposure:min:0.9,max:1" --target-bounds "OUTCOME:min:0.9,max:1" --target-bounds "OUTCOME_2:min:0.8,max:1" --target-bounds "OUTCOME_3:min:0.7,max:1"
if errorlevel 1 goto :error
