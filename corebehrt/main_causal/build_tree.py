...
def main():
    ...
    parser = argparse.ArgumentParser(description='Build diagnosis tree at specified level')
    parser.add_argument(
        '--level', 
        type=int, 
        default=3,
    ...
    if args.type == 'diagnosis':
        csv_file = (
            f"{SKS_DUMP_DIR}/sks_dump_diag.csv"  # Adjust the path as needed.
        )
    elif args.type == 'medication':
        csv_file = (
            f"{SKS_DUMP_DIR}/sks_dump_med.csv"  # Adjust the path as needed.
        )
