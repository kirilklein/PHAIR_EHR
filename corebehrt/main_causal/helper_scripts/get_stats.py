...
def process_cohort(
    ...
    if cfg.get("common_support_threshold", None) is not None:
        ...
        criteria_processed = filter_common_support(
            ...
            threshold=cfg.get("common_support_threshold"),
        )
        filtered = True
    ...
    if cfg.get("weights", None) is not None:
        ...
        criteria_processed[WEIGHTS_COL] = compute_weights(criteria_processed, weights)
        weighted_stats = analyze_cohort_with_weights(criteria_processed, WEIGHTS_COL)
        if cohort_name:
            logger.info(f"--------------------------------")
            logger.info(f"Weighted stats ({weights}) for {cohort_name}:")
        else:
            logger.info("--------------------------------")
            logger.info(f"Weighted stats ({weights}):")
