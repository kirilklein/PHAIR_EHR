...
class Node:
    ...
    def sum_counts(self)->int:
        self.sum_count : int = self.base_count + sum(child.sum_counts() for child in self.children)
    ...
    def redist_counts(self):
        ...
        for child in self.children:                                         # sum of children
            child.redist_count = self.redist_count * (child.sum_count / (self.sum_count-self.base_count))
            child.redist_counts()
    ...
    def create_target_mapping(self, value:int=-100)->dict:
        ...
        for level in range(1, max_level+1):
            ...
            for i, node in enumerate(nodes):
                mapping[node.name] = mapping[node.parent.name][:level-1] + [i] + [value] * (max_level - level)  # parent mapping + index + padding
    ...
    def get_level(self, level:int)->list:
        if self.parent is None and level > self.get_max_level():
            raise IndexError(f'Level {level} is too high. Max level is {self.get_max_level()}')
        if level == 0:
            return [self]
        else:
    ...
    def get_leaf_counts(self)->torch.Tensor:
        return torch.tensor([c.redist_count for c in self.get_level(self.get_max_level())])
