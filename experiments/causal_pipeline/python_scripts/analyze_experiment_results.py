...
def load_experiment_results(
    ...
    for exp_dir in exp_dirs:
        run_id = (
            exp_dir.parent.name if exp_dir.parent.name.startswith("run_") else "run_01"
        )
...
def create_plots(df: pd.DataFrame, output_dir: str):
    ...
    unique_combinations = (
        methods_df[["i", "y"]].drop_duplicates().sort_values(["i", "y"])
    )
    ...
    for metric, title_prefix, ylabel in plot_configs:
        ...
        fig.suptitle(
            f"{title_prefix} by Instrument/Outcome-Only Combinations",
            fontsize=20,
            fontweight="bold",
        )

        for i, (param, xlabel_base) in enumerate(
            [
                ("avg_confounding", "Confounding"),
                ("i", "Instrument"),
                ("y", "Outcome-Only"),
            ]
        ):
            ...
            for combo_idx, (_, combo_row) in enumerate(unique_combinations.iterrows()):
                ...
                combo_data = methods_df[
                    (methods_df["i"] == i_val) & (methods_df["y"] == y_val)
                ]
            ...
            if metric == "covered":
                ax.axhline(
                    0.95, color="black", linestyle=":", alpha=0.7, label="95% Target"
                )
                ax.set_ylim(0, 1.05)

            ax.set_title(
                f"{title_prefix} vs. {xlabel_base}", fontsize=14, fontweight="bold"
            )
