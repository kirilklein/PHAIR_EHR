...
def create_experiment_template(experiment_name, script_dir):
    ...
    if output_path.exists():
        response = input(
            f"Experiment '{experiment_name}' already exists. Overwrite? (y/N): "
        )
        if response.lower() != "y":
            print("Cancelled.")
            return
    ...
    template_config = {
        ...
        "simulation_model": {
            ...
            "factor_mapping": {"mean": 0.0, "scale": 1.4, "sparsity_factor": 0.8},
            "influence_scales": {
        ...
        "outcomes": {
            ...
            "OUTCOME_NULL": {"run_in_days": 1, "p_base": 0.2, "exposure_effect": 0.0},
        },
    }
...
def main():
    ...
    parser.add_argument(
        "experiment_name", help="Name of the experiment (use underscores, no spaces)"
    )
