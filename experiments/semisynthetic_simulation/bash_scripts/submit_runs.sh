...
for ((i = 1; i <= N_RUNS; i++)); do
  ...
  BASH_ARGS="--run-id $RUN --inner-runs $INNER_RUNS --n-bootstrap $N_BOOTSTRAP ${EXTRA_ARGS[*]:-}"

  echo "Submitting $RUN  (refits=$INNER_RUNS, bootstraps=$N_BOOTSTRAP) ${EXTRA_ARGS[*]:-}"
