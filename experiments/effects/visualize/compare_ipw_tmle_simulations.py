...
def create_comparison_plot(metrics_original: pd.DataFrame, 
    ...
    # Combine datasets
    combined_metrics = pd.concat([metrics_original, metrics_ipw_favorable], 
                                ignore_index=True)
    ...
    sns.barplot(data=combined_metrics, 
                x='outcome', y='abs_bias', hue='method', 
                col='simulation', kind='bar',
                ax=axes[0, 0])
    ...
    avg_bias = combined_metrics.groupby(['simulation', 'method'])['abs_bias'].mean().reset_index()
    sns.barplot(data=avg_bias, x='simulation', y='abs_bias', hue='method', ax=axes[1, 0])
    ...
    pivot_original = metrics_original.pivot(index='outcome', columns='method', values='abs_bias')
    pivot_ipw = metrics_ipw_favorable.pivot(index='outcome', columns='method', values='abs_bias')
    
    if 'IPW' in pivot_original.columns and 'TMLE' in pivot_original.columns:
        axes[1, 1].scatter(pivot_original['IPW'], pivot_original['TMLE'], 
                          alpha=0.7, label='Original', s=80)
    if 'IPW' in pivot_ipw.columns and 'TMLE' in pivot_ipw.columns:
        axes[1, 1].scatter(pivot_ipw['IPW'], pivot_ipw['TMLE'], 
                          alpha=0.7, label='IPW-favorable', s=80)
...
def main():
    ...
    try:
        ...
        create_comparison_plot(metrics_original, metrics_ipw_favorable,
        ...
        orig_summary = metrics_original.groupby('method')['abs_bias'].agg(['mean', 'std'])
        print(orig_summary)
        
        print("\nIPW-favorable Simulation (Pure Confounding Structure):")
        ipw_summary = metrics_ipw_favorable.groupby('method')['abs_bias'].agg(['mean', 'std'])
        print(ipw_summary)
