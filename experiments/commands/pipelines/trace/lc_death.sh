python -m corebehrt.azure pipeline FINETUNE_ESTIMATE CPU-300 azure_configs/trace/pipeline \
  ...
  --outcomes       researcher_data:AKK/shared/outcomes/trace/death \
  --exposures      researcher_data:AKK/shared/outcomes/trace/lung_cancer \
  -cp finetune_exp_y=GPU-A100-small \
  -e  trace_ppl_lc_death_test_august_v01 \
  -c prepare_finetune_exp_y=azure_configs/trace/pipeline/prepare/lc_2y.yaml \
  -c select_cohort_full=azure_configs/trace/pipeline/cohort/lc.yaml
